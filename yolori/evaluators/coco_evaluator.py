...
from yolori.data.datasets import COCO_CLASSES
from yolori.data.datasets.dior_classes import DIOR_CLASSES
from yolori.data.datasets.classes import NWPU_CLASSES
...
class DIOREvaluator:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            info += redirect_string.getvalue()
            if self.per_class_mAP:
                info += "per class mAP:\n" + per_class_mAP_table(cocoEval, class_names=DIOR_CLASSES)
            return cocoEval.stats[0], cocoEval.stats[1], info
        else:
            return 0, 0, info
...
class NWPUEvaluator:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            info += redirect_string.getvalue()
            if self.per_class_mAP:
                info += "per class mAP:\n" + per_class_mAP_table(cocoEval, class_names=NWPU_CLASSES)
            return cocoEval.stats[0], cocoEval.stats[1], info
        else:
            return 0, 0, info
