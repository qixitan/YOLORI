...
class Scale_Aware_Layer(nn.Module):
    ...
    def forward(self, F):
        # Transposing input from (batch_size, L, S, C) to (batch_size, S, L, C)
        # so we can use convolutional layer over the level dimension L
...
class Spatial_Aware_Layer(nn.Module):
    ...
    def __init__(self, L_size, kernel_height=3, kernel_width=3, padding=1, stride=1, dilation=1, groups=1):
        ...
        self.offset_and_mask_conv = nn.Conv2d(in_channels=self.in_channels, out_channels=3 * self.K,  # 3K depth
...
def DyHead(num_blocks, L, S, C):
    blocks = [DyHead_Block(L, S, C) for _ in range(num_blocks)]
