...
__all__ = [
    "BlurPool",
    "BlurPool1D",
    "Downsample_PASA_group_softmax"
]


class BlurPool(nn.Module):
    def __init__(self, channels, pad_type='reflect', kernel_size=3, stride=2, pad_off=0):
        super(BlurPool, self).__init__()
        self.filt_size = kernel_size
        self.pad_off = pad_off
        self.pad_sizes = [int(1.*(kernel_size-1)/2), int(np.ceil(1.*(kernel_size-1)/2)), int(1.*(kernel_size-1)/2), int(np.ceil(1.*(kernel_size-1)/2))]
...
class BlurPool1D(nn.Module):
    def __init__(self, channels, pad_type='reflect', kernel_size=3, stride=2, pad_off=0):
        ...
        self.filt_size = kernel_size
        self.pad_off = pad_off
        self.pad_sizes = [int(1. * (kernel_size - 1) / 2), int(np.ceil(1. * (kernel_size - 1) / 2))]
        self.pad_sizes = [pad_size + pad_off for pad_size in self.pad_sizes]
        self.stride = stride
        self.off = int((self.stride - 1) / 2.)
        self.channels = channels
